...
return dDisplay + hDisplay + mDisplay + sDisplay;
  ...
  ttdl: async function ttdl(url) {
    return new Promise(async (resolve, reject) => {
      try {
        const axios = require("axios"); // Ensure axios is installed in your project
        const apiKey = "nikka"; // Replace with your actual API key
        const apiUrl = `https://nikka-api.us.kg/dl/tiktok?url=${encodeURIComponent(url)}&apiKey=${apiKey}`;
        
        const response = await axios.get(apiUrl);
        if (response.data) {
          resolve(response.data); // Return the full response data
        } else {
          reject("No response data received from the API.");
        }
      } catch (error) {
        reject(`Error occurred while downloading TikTok video: ${error.message}`);
      }
    });
  },
  yts: async function yts(query) {
    return new Promise(async (resolve, reject) => {
      try {
        const axios = require("axios"); // Ensure axios is installed
        const apiKey = "nikka"; // Replace with your actual API key
        const apiUrl = `https://nikka-api.us.kg/search/yts?q=${encodeURIComponent(query)}&apiKey=${apiKey}`;
        
        const response = await axios.get(apiUrl);
        if (response.data) {
          resolve(response.data); // Return the full response data
        } else {
          reject("No response data received from the YouTube search API.");
        }
      } catch (error) {
        reject(`Error occurred while performing YouTube search: ${error.message}`);
      }
    });
  },
  ytd: async function ytd(query) {
    return new Promise(async (resolve, reject) => {
      try {
        const axios = require("axios"); // Ensure axios is installed
        const apiKey = "nikka"; // Replace with your actual API key
        const apiUrl = `https://nikka-api.us.kg/dl/yt?q=${encodeURIComponent(query)}&apiKey=${apiKey}`;
